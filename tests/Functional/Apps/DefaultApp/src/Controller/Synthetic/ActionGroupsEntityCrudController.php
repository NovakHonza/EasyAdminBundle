 ...
 */
class ActionGroupsEntityCrudController extends AbstractCrudController
{
    ...
    public function configureActions(Actions $actions): Actions
    {
        ...
        // conditional display based on isActive status
        $group6 = ActionGroup::new('group6', 'Action Group 6')
            ->displayIf(static fn ($entity): bool => $entity instanceof ActionTestEntity && $entity->isActive())
            ->addAction(Action::new('action1', 'Action 1')->linkToCrudAction('edit'))
            ->addAction(Action::new('action2', 'Action 2')->linkToCrudAction('detail'));
        ...
        // action groups for NEW page (actions that don't require entityId)
        $groupNew1 = ActionGroup::new('groupNew1', 'New Page Group 1')
            ->addAction(Action::new('action1', 'Back to List')->linkToCrudAction('index'))
            ->addAction(Action::new('action2', 'Help')->linkToUrl('#help'))
            ->addAction(Action::new('action3', 'Settings')->linkToUrl('#settings'));

        $groupNew2 = ActionGroup::new('groupNew2', 'New Page Group 2')
            ->addHeader('Quick Links')
            ->addAction(Action::new('link1', 'Documentation')->linkToUrl('#docs'))
            ->addAction(Action::new('link2', 'Dashboard')->linkToRoute('admin'))
            ->addDivider()
            ->addHeader('More')
            ->addAction(Action::new('link3', 'External')->linkToUrl('https://example.com'));

        $groupNew3 = ActionGroup::new('groupNew3', t('New Page Group 3'))
            ->addHeader(t('Options'))
            ->addAction(Action::new('opt1', t('Option 1'))->linkToUrl('#opt1'))
            ->addAction(Action::new('opt2', t('Option 2'))->linkToUrl('#opt2'));
        ...
        return $actions
            ...
            ->add(Crud::PAGE_NEW, $groupNew1)
            ->add(Crud::PAGE_NEW, $groupNew2)
            ->add(Crud::PAGE_NEW, $groupNew3)
